  ...
  return withBusinessContext(business, async (client) => {
    ...
    if (existing?.shopify_id) {
      response = await axios.put(
        ...
        shopifyProduct, { headers: HEADERS }
      );
    } else {
      response = await axios.post(`${BASE}/products.json`, shopifyProduct, { headers: HEADERS });
      ...
      await client.query(
        ...
        [JSON.stringify({ ...current, shopify_product_id: shopifyId }), productId]
      );
    }
...
async function syncStockLevel(business, productId, quantity) {
  return withBusinessContext(business, async (client) => {
    ...
    // Get location ID
    const locRes = await axios.get(`${BASE}/locations.json`, { headers: HEADERS });
