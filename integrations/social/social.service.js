...
async function publishPost({ channels = [], imageUrls = [], caption, videoPath, title, description }) {
  const results = [];
  ...
  for (const channel of channels) {
    ...
    try {
      ...
      if (channel === 'instagram') {
        postId = imageUrls.length > 1
          ? await instagram.createCarouselPost({ imageUrls, caption })
          : await instagram.createImagePost({ imageUrl: imageUrls[0], caption });
      } else if (channel === 'facebook') {
        postId = await facebook.createPost({ message: caption, imageUrl: imageUrls[0] });
      } else if (channel === 'tiktok' && videoPath) {
        postId = await tiktok.uploadVideo({ filePath: videoPath, title, description });
      } else if (channel === 'youtube' && videoPath) {
        postId = await youtube.uploadVideo({ filePath: videoPath, title, description });
      }
