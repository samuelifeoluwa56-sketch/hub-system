...
async function createShipment({ senderAddress, receiverAddress, items, reference, weight }) {
  if (!accessToken) await authenticate();

  const { data } = await axios.post(`${BASE}/shipments`, {
...
async function trackShipment(waybill) {
  if (!accessToken) await authenticate();
  const { data } = await axios.get(`${BASE}/shipments/${waybill}/track`, { headers: headers() });
  return { status: data.Status, location: data.CurrentLocation, history: data.History };
}
