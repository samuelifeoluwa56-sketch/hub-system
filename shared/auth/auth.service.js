...
async function logout(userId, rawRefreshToken) {
  return withSharedContext(async (client) => {
    ...
    await client.query(
      `DELETE FROM shared.user_sessions WHERE user_id = $1`, [userId]
    );
  });
}
...
async function switchBusiness(userId, business) {
  if (!config.app.businesses.includes(business)) {
    throw Object.assign(new Error(`Invalid business: ${business}`), { status: 400 });
  }
  ...
  return withSharedContext(async (client) => {
    ...
    if (!rows.length || !rows[0].permitted_businesses.includes(business)) {
      throw Object.assign(new Error('Not permitted for this business'), { status: 403 });
    }
...
async function changePassword(userId, currentPassword, newPassword) {
  return withSharedContext(async (client) => {
    const { rows } = await client.query(
      `SELECT password_hash FROM shared.users WHERE user_id = $1`, [userId]
    );
    if (!rows.length) throw Object.assign(new Error('User not found'), { status: 404 });
    ...
    const { rows: roleRows } = await client.query(
      `SELECT role_id FROM shared.user_roles WHERE user_id = $1 LIMIT 1`, [userId]
    );
    if (roleRows.length) await invalidatePermissionCache(roleRows[0].role_id);
  });
}

module.exports = { login, refresh, logout, switchBusiness, getMe, changePassword };
