...
// GET /api/contacts/:id
router.get('/:id', param('id').isUUID(), validate, can('crm','view'), async (req, res, next) => {
  try {
...
router.patch('/:id', param('id').isUUID(), validate, can('crm','edit'), async (req, res, next) => {
  try {
...
router.delete('/:id', param('id').isUUID(), validate, can('crm','delete'), async (req, res, next) => {
  try {
...
router.get('/:id/timeline', param('id').isUUID(), validate, can('crm','view'), async (req, res, next) => {
  try {
...
router.post('/:id/addresses',
  ...
  async (req, res, next) => {
    try {
      const address = await service.addAddress(req.params.id, req.body, req.user);
