...
router.get('/:productId/movements', param('productId').isUUID(), validate, can('stock','view'), async (req, res, next) => {
  try {
    const result = await service.getMovements(req.business, req.params.productId, req.query);
    res.json(result);
  } catch (err) { next(err); }
});
...
router.post('/adjustment',
  ...
  validate, can('stock','approve'),
  async (req, res, next) => {
    try {
      const result = await service.createAdjustment(req.business, req.body, req.user);
...
router.post('/transfer',
  ...
  validate, can('stock','create'),
  async (req, res, next) => {
    try {
      const result = await service.createTransfer(req.business, req.body, req.user);
      res.status(201).json(result);
    } catch (err) { next(err); }
  }
);
