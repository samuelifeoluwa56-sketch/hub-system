...
async function listQuotations(business, { page=1, limit=50, status, contactId }={}, user, scope) {
  ...
  return withBusinessContext(business, async (client) => {
    ...
    const { rows: [q] } = await client.query(
      `INSERT INTO quotations
       ...
       RETURNING *`,
      [quoteNumber, data.contact_id, data.deal_id||null, data.assigned_to||user.user_id,
       data.valid_until, subtotal, discountTotal, vatTotal, total,
       data.payment_terms||null, data.notes||null, data.terms_conditions||null, user.user_id]
    );
    ...
    for (let i=0; i<data.lines.length; i++) {
      ...
      await client.query(
        ...
        [q.quotation_id, l.product_id||null, l.description, l.quantity,
         l.unit_price, l.discount_pct||0, disc, lt-disc, i]
      );
    }

    if (data.deal_id) {
      await crmService.logActivity(business, data.deal_id, {
        activity_type: 'quotation_sent', summary: `Quotation ${quoteNumber} created`, is_auto: true,
      }, user, client);
    }
...
async function getQuotation(business, quotationId) {
  return withBusinessContext(business, async (client) => {
    ...
    if (!q) throw Object.assign(new Error('Quotation not found'), { status: 404 });
    return q;
  });
}
...
async function updateQuotation(business, quotationId, data, user) {
  return withBusinessContext(business, async (client) => {
    const { rows: [q] } = await client.query(
      `SELECT status FROM quotations WHERE quotation_id = $1`, [quotationId]
    );
    if (!q) throw Object.assign(new Error('Not found'), { status: 404 });
    if (!['draft'].includes(q.status)) {
      throw Object.assign(new Error('Only draft quotations can be edited'), { status: 400 });
    }
    const allowed = ['valid_until','payment_terms','notes','terms_conditions'];
    const sets=[], vals=[];
    ...
    const { rows: [updated] } = await client.query(
      `UPDATE quotations SET ${sets.join(', ')}, updated_at = now()
       WHERE quotation_id = $${vals.length} RETURNING *`, vals
    );
    return updated;
  });
}

async function sendQuotation(business, quotationId, { channel='email' }, user) {
  ...
  if (channel === 'email' && q.email) {
    await sendEmail({
      ...
      attachments: [{ filename: `${q.quotation_number}.pdf`, content: pdf, contentType: 'application/pdf' }],
    });
  ...
  await withBusinessContext(business, async (client) => {
    await client.query(
      `UPDATE quotations SET status='sent', sent_at=now()
       WHERE quotation_id=$1 AND status='draft'`, [quotationId]
    );
  });
...
async function confirmQuotation(business, quotationId, user) {
  return withBusinessContext(business, async (client) => {
    ...
    if (!['sent','viewed','draft'].includes(q.status)) {
      throw Object.assign(new Error('Quotation cannot be confirmed'), { status: 400 });
    }
    ...
    const { rows: [order] } = await client.query(
      ...
      [orderNumber, quotationId, q.contact_id,
       q.fulfilment_type||'walk_in', q.total_amount, user.user_id]
    );
    ...
    await client.query(
      `UPDATE quotations SET status='confirmed', confirmed_at=now()
       WHERE quotation_id=$1`, [quotationId]
    );
...
module.exports = { listQuotations, createQuotation, getQuotation, updateQuotation,
                   sendQuotation, confirmQuotation, listOrders, getOrder, generateQuotationPDF };
