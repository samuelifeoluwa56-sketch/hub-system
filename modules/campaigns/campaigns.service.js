...
async function list(business, { page = 1, limit = 30, status, campaign_type } = {}) {
...
async function create(business, data, user) {
  return withBusinessContext(business, async (client) => {
    ...
    await auditService.log(client, {
      ...
      module: 'campaigns', action: 'create',
      table: 'campaigns', recordId: campaign.campaign_id, after: campaign,
    });
...
async function update(business, campaignId, data, user) {
  return withBusinessContext(business, async (client) => {
    const { rows: [c] } = await client.query(
      `SELECT status FROM campaigns WHERE campaign_id=$1`, [campaignId]
    );
    if (!c) throw Object.assign(new Error('Not found'), { status: 404 });
    if (!['draft'].includes(c.status)) {
      throw Object.assign(new Error('Only draft campaigns can be edited'), { status: 400 });
    }

    const allowed = ['campaign_name','subject_line','from_name','html_content','audience_filter'];
    const sets = [], vals = [];
    ...
    const { rows: [updated] } = await client.query(
      `UPDATE campaigns SET ${sets.join(', ')}, updated_at=now()
       WHERE campaign_id=$${vals.length} RETURNING *`, vals
    );
    return updated;
  });
}
...
async function buildAudience(business, campaignId) {
  return withBusinessContext(business, async (client) => {
    ...
    return { recipient_count: inserted, message: 'Audience built successfully' };
  });
}
...
async function schedule(business, campaignId, scheduledAt, user) {
  return withBusinessContext(business, async (client) => {
    ...
    const { rows: [c] } = await client.query(
      `SELECT status, recipient_count FROM campaigns WHERE campaign_id=$1`, [campaignId]
    );
    if (!c) throw Object.assign(new Error('Not found'), { status: 404 });
    if (!['draft'].includes(c.status)) {
      throw Object.assign(new Error('Only draft campaigns can be scheduled'), { status: 400 });
    }
    if (!c.recipient_count) {
      throw Object.assign(new Error('Build audience first before scheduling'), { status: 400 });
    }
...
async function sendNow(business, campaignId, user) {
  return withBusinessContext(business, async (client) => {
    ...
    if (!campaign) throw Object.assign(new Error('Campaign not sendable'), { status: 400 });
    ...
    setImmediate(() => processSend(business, campaignId, campaign).catch(err => {
...
async function processSend(business, campaignId, campaign) {
  ...
  await withBusinessContext(business, async (client) => {
    ...
    for (const r of recipients) {
      try {
        let personalHtml = campaign.html_content
          .replace(/\{\{name\}\}/g, r.display_name || 'Valued Customer');
    ...
    logger.info(`Campaign sent: ${campaignId} — ${sentCount}/${recipients.length} delivered`);
  });
}
...
async function cancel(business, campaignId, user) {
  return withBusinessContext(business, async (client) => {
    ...
    if (!c) throw Object.assign(new Error('Campaign cannot be cancelled'), { status: 400 });
    return c;
  });
}
...
module.exports = { list, create, getById, update, buildAudience,
                   schedule, sendNow, cancel, getStats, trackEvent };
