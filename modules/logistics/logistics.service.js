...
async function listDeliveries(business, { page = 1, limit = 50, status, courier } = {}) {
  ...
  return withBusinessContext(business, async (client) => {
    ...
    const { rows: [delivery] } = await client.query(
      `INSERT INTO deliveries
       ...
       RETURNING *`,
      [
        deliveryNumber, data.reference_type, data.reference_id, data.contact_id,
      ...
      for (const item of data.items) {
        await client.query(
          `INSERT INTO delivery_items (delivery_id, product_id, description, quantity)
           VALUES ($1,$2,$3,$4)`,
          [delivery.delivery_id, item.product_id || null, item.description, item.quantity]
        );
      }
    } else {
    ...
    await auditService.log(client, {
      ...
      table: 'deliveries', recordId: delivery.delivery_id, after: delivery,
    });
...
async function dispatchDelivery(business, deliveryId, user) {
  return withBusinessContext(business, async (client) => {
    ...
    if (!delivery) throw Object.assign(new Error('Delivery not found or not ready to dispatch'), { status: 400 });
    ...
    const booking = await courierService.bookCourier({
      ...
      contact:  { display_name: delivery.display_name, primary_phone: delivery.primary_phone },
      items:    items.map(i => ({ description: i.description, quantity: i.quantity, weight_grams: i.weight_grams })),
    });
    ...
    if (delivery.whatsapp_number) {
      ...
      }).catch(err => logger.warn('WhatsApp dispatch notification failed', err));
    }

    emitToBusiness(business, 'delivery:dispatched', {
      deliveryId, courierId: booking.courierId, status: 'dispatched',
    });
...
async function markDelivered(business, deliveryId, user) {
  return withBusinessContext(business, async (client) => {
    ...
    if (!delivery) throw Object.assign(new Error('Delivery not found or cannot be marked delivered'), { status: 400 });
...
async function markFailed(business, deliveryId, { failure_reason }, user) {
  return withBusinessContext(business, async (client) => {
    ...
    if (!delivery) throw Object.assign(new Error('Cannot update this delivery'), { status: 400 });
    ...
    for (const m of managers) {
      await notifService.create(client, {
        userId: m.user_id, business, type: 'delivery_update',
...
async function getQuote({ courier, pickup_address, delivery_address }) {
  return courierService.getQuote({ courier, pickupAddress: pickup_address, deliveryAddress: delivery_address });
}

module.exports = { listDeliveries, getDelivery, createDelivery, dispatchDelivery,
                   markDelivered, markFailed, getTracking, getQuote };
