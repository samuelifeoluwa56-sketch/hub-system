...
async function openSession(business, { terminal_id, opening_float = 0 }, user) {
  return withBusinessContext(business, async (client) => {
    ...
    if (existing.length) throw Object.assign(new Error('Terminal already has an open session'), { status: 409 });
    ...
    emitToBusiness(business, 'pos:session_opened', { sessionId: session.session_id, terminalId: terminal_id });
    return session;
  });
}
...
async function closeSession(business, sessionId, { actual_cash, reconciliation_notes }, user) {
  return withBusinessContext(business, async (client) => {
    ...
    const { rows: [session] } = await client.query(
      ...
      [user.user_id, actual_cash, totals.cash_total,
       totals.transfer_total, totals.card_total, totals.total_revenue,
       reconciliation_notes||null, sessionId]
    );
    if (!session) throw Object.assign(new Error('Session not found or already closed'), { status: 400 });
    ...
    await client.query(
      ...
      [sessionId, txCount.total, txCount.voided,
       totals.total_revenue, totals.cash_total, totals.card_total, totals.transfer_total]
    );
...
async function createTransaction(business, data, user) {
  return withBusinessContext(business, async (client) => {
    ...
    if (!session) throw Object.assign(new Error('No open session found'), { status: 400 });

    // Check minimum price per line
    for (const line of data.lines) {
      if (!line.product_id) continue;
      const { rows: [prod] } = await client.query(
        `SELECT min_selling_price FROM products WHERE product_id=$1`, [line.product_id]
      );
      if (prod?.min_selling_price && parseFloat(line.unit_price) < parseFloat(prod.min_selling_price)) {
        const { rows: [da] } = await client.query(
          ...
          [data.session_id, line.product_id, line.unit_price, prod.min_selling_price, user.user_id]
        );
        // Notify managers
        ...
        for (const m of managers) {
          await notifService.create(client, {
            userId: m.user_id, business, type: 'approval_required',
            title: 'Discount approval required',
            body: `Staff requesting price below minimum. Approval ID: ${da.approval_id}`,
            referenceType: 'discount_approval', referenceId: da.approval_id,
          });
        }
        throw Object.assign(new Error(`Price below minimum. Approval requested: ${da.approval_id}`), { status: 402, approvalId: da.approval_id });
      }
    }
    ...
    const { rows: [tx] } = await client.query(
      ...
      [txNumber, data.session_id, data.contact_id||null, user.user_id,
       subtotal, discountTotal, vatTotal, totalAmount,
       totalPaid, change, data.fulfilment_type||'walk_in']
    );
    ...
    for (let i = 0; i < data.lines.length; i++) {
      ...
      await client.query(
        ...
        [tx.transaction_id, l.product_id||null, l.description, l.quantity,
         l.unit_price, l.discount_amount||0, vat, net+vat, i]
      );
      if (l.product_id) {
        await stockService.recordMovement(client, {
          business, productId: l.product_id, movementType: 'pos_sale',
          quantity: l.quantity, direction: -1,
          referenceType: 'pos_transaction', referenceId: tx.transaction_id,
    ...
    for (const p of data.payments) {
      await client.query(
        `INSERT INTO pos_payment_splits
         ...
         VALUES ($1,$2,$3,$4,$5,$6)`,
        [tx.transaction_id, p.payment_method, p.amount,
         p.reference||null, p.paystack_reference||null,
         p.payment_method !== 'bank_transfer']
      );
    }

    // Update session running totals
    const transferAmt = data.payments.filter(p=>p.payment_method==='bank_transfer').reduce((s,p)=>s+parseFloat(p.amount),0);
    const cardAmt     = data.payments.filter(p=>p.payment_method==='pos_card').reduce((s,p)=>s+parseFloat(p.amount),0);
    await client.query(
      `UPDATE pos_sessions
    ...
    await auditService.log(client, {
      ...
      table: 'pos_transactions', recordId: tx.transaction_id, after: tx,
    });

    emitToBusiness(business, 'pos:transaction_completed', { transactionId: tx.transaction_id, amount: totalAmount });
...
async function voidTransaction(business, transactionId, { void_reason }, user) {
  return withBusinessContext(business, async (client) => {
    ...
    if (!tx) throw Object.assign(new Error('Transaction cannot be voided'), { status: 400 });
    ...
    for (const l of lines) {
      await stockService.recordMovement(client, {
        business, productId: l.product_id, movementType: 'returned_from_customer',
        quantity: l.quantity, direction: 1,
        referenceType: 'pos_transaction', referenceId: transactionId,
        performedBy: user.user_id, notes: `Void: ${void_reason}`,
      });
    }
    ...
    await auditService.log(client, {
      ...
      table: 'pos_transactions', recordId: transactionId, before: tx,
    });
...
module.exports = {
  getTerminals, openSession, getSession, closeSession,
  createTransaction, getTransaction, voidTransaction, sendReceipt,
};
