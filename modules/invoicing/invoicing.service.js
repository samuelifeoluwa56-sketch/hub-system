...
async function list(business, { page = 1, limit = 50, status, contactId }, scope, user) {
  return withBusinessContext(business, async (client) => {
    ...
    const { rows } = await client.query(
      `SELECT i.invoice_id, i.invoice_number, i.invoice_type, i.status,
       ...
       LIMIT $5 OFFSET $6`,
      [status || null, contactId || null, scope, user.user_id, parseInt(limit), offset]
    );
    return { data: rows };
  });
}
...
async function create(business, data, user) {
  return withBusinessContext(business, async (client) => {
    ...
    for (const line of data.lines) {
      const lineTotal = line.unit_price * line.quantity - (line.discount_amount || 0);
    ...
    const { rows: [inv] } = await client.query(
      `INSERT INTO invoices
       ...
       RETURNING *`,
      [
        invoiceNumber, data.invoice_type || 'standard', data.contact_id,
        data.order_id || null, data.due_date, subtotal,
        data.discount_total || 0, vatTotal, total,
        data.currency || 'NGN', data.notes || null,
        data.payment_instructions || null, user.user_id,
      ]
    );
    ...
    for (let i = 0; i < data.lines.length; i++) {
      ...
      await client.query(
        ...
        [inv.invoice_id, l.product_id || null, l.description,
         l.quantity, l.unit_price, l.discount_amount || 0,
         l.vat_rate || 0.075, vatAmt, lineTotal + vatAmt, i]
      );
    }
    ...
    await auditService.log(client, {
      ...
      table: 'invoices', recordId: inv.invoice_id, after: inv,
    });
...
async function postInvoiceJournal(client, business, invoice) {
  ...
  const { rows: [ar] }  = await client.query(`SELECT account_id FROM chart_of_accounts WHERE account_code = '1310' LIMIT 1`);
  const { rows: [rev] } = await client.query(`SELECT account_id FROM chart_of_accounts WHERE account_code = '4100' LIMIT 1`);
  const { rows: [vat] } = await client.query(`SELECT account_id FROM chart_of_accounts WHERE account_code = '2210' LIMIT 1`);
  ...
  const { rows: [entry] } = await client.query(
    `INSERT INTO journal_entries
     ...
     RETURNING entry_id`,
    [`Invoice ${invoice.invoice_number}`, invoice.invoice_id, invoice.created_by]
  ...
  await client.query(
    `INSERT INTO journal_lines (entry_id, account_id, debit, credit) VALUES
     ...
     ($1, $4, 0, $5),
     ($1, $6, 0, $7)`,
    [entry.entry_id, ar.account_id, invoice.total_amount,
     rev.account_id, invoice.subtotal,
     vat.account_id, invoice.vat_amount]
  );
}
  ...
  return withBusinessContext(business, async (client) => {
    const { rows: [payment] } = await client.query(
      `INSERT INTO invoice_payments
       ...
       RETURNING *`,
      [
        invoiceId, data.payment_date || null, data.amount,
        data.payment_method, data.reference || null,
        data.paystack_reference || null,
        data.is_confirmed !== undefined ? data.is_confirmed : true,
        user.user_id, data.notes || null,
      ]
    );
    ...
    const { rows: [inv] } = await client.query(
      `SELECT invoice_number, contact_id FROM invoices WHERE invoice_id = $1`, [invoiceId]
    );

    await auditService.log(client, {
      userId: user.user_id, userName: 'staff', business,
      module: 'invoicing', action: 'create',
      table: 'invoice_payments', recordId: payment.payment_id, after: payment,
    });
...
async function voidInvoice(business, invoiceId, user) {
  return withBusinessContext(business, async (client) => {
    ...
    if (!inv) throw Object.assign(new Error('Cannot void this invoice'), { status: 400 });

    await auditService.log(client, {
      userId: user.user_id, userName: 'staff', business,
      module: 'invoicing', action: 'delete',
      table: 'invoices', recordId: invoiceId, before: inv,
    });
  });
}
...
module.exports = { list, getById, create, recordPayment, send, voidInvoice, generatePDF };
