...
router.get('/', can('invoicing','view'), async (req, res, next) => {
  try {
    const result = await service.list(req.business, req.query, req.permissionScope, req.user);
    res.json(result);
  } catch (err) { next(err); }
});

// GET /api/invoicing/:id
router.get('/:id', param('id').isUUID(), validate, can('invoicing','view'), async (req, res, next) => {
  try {
...
router.post('/:id/send', param('id').isUUID(), validate, can('invoicing','edit'), async (req, res, next) => {
  try {
...
router.post('/:id/payments',
  ...
  validate, can('invoicing','edit'),
  async (req, res, next) => {
    try {
      const payment = await service.recordPayment(req.business, req.params.id, req.body, req.user);
...
router.post('/:id/void', param('id').isUUID(), validate, can('invoicing','delete'), async (req, res, next) => {
  try {
...
router.get('/:id/pdf', param('id').isUUID(), validate, can('invoicing','view'), async (req, res, next) => {
  try {
