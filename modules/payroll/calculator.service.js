 ...
 */
async function calculatePayslip(business, profileId, periodMonth, periodYear, client) {
  ...
  const housingAllowance   = parseFloat((basicSalary * 0.20).toFixed(2)); // 20% of basic
  const transportAllowance = parseFloat((basicSalary * 0.10).toFixed(2)); // 10% of basic

  // Commission earned this period
  const { rows: [commRow] } = await client.query(
  ...
  const grossSalary = parseFloat(
    (basicSalary + housingAllowance + transportAllowance + commissionAmount).toFixed(2)
  );
  ...
  const { rows: [absRow] } = await client.query(
