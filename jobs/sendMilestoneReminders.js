  ...
  for (const business of ['jewelry', 'diffusers']) {
    await withBusinessContext(business, async (client) => {
      ...
      for (const row of rows) {
        ...
        emitToBusiness(business, 'notification:new', { userId: row.assigned_to });
        logger.info(`Milestone reminder queued: ${row.display_name} [${row.milestone_type}]`);
      }
    });
  }
};
